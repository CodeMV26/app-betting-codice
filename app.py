...
# --- RESTYLING GRAFICO ULTRA-OTTIMIZZATO (VERSIONE 5.31) ---
st.markdown(f"""
    <style>
...
<div class="brand-box">
    ...
    <div class="version-label">Versione Progetto: 5.31</div>
</div>
""", unsafe_allow_html=True)
...
if st.button(testo_p3, key="fase_3_btn", use_container_width=True):
    with st.spinner("⏳ In corso..."):
        try:
            ...
            # Allineamento del nome funzione a quella reale presente in modulo_04_trasferitore.py
...
st.markdown(f'<div class="debug-badge">iPhone X Colors Locked 5.31 | Active Tab: {st.session_state.tab_selezionata}</div>', unsafe_allow_html=True)
